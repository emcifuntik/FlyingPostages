...
class Admin
{
    static public function change_user_group($id, $group)
    {
        ...
        $prep_query = $mysql->prepare("UPDATE users,groups SET users.`user_group`=? WHERE users.`user_id`=? AND groups.`group_id`=?") //group exist
    ...
    {
        $mysql = MySQL::GetInstance();
        $prep_query = $mysql->prepare("UPDATE users SET `user_money`=`user_money` + ? WHERE `user_id`=?")
        or die('MySQL Error. Error[' . $mysql->errno . ']: ' . $mysql->error);
    ...
    {
        $mysql = MySQL::GetInstance();
        $prep_query = $mysql->prepare("UPDATE users SET `user_money`=`user_money` - ? WHERE `user_id`=? AND `user_money` >= ?")
        or die('MySQL Error. Error[' . $mysql->errno . ']: ' . $mysql->error);
    ...
    {
        $mysql = MySQL::GetInstance();
        $prep_query = $mysql->prepare("INSERT INTO `groups` (`group_name`, `group_color`, `group_package_price`," .
    ...
    {
        $mysql = MySQL::GetInstance();
        $prep_query = $mysql->prepare("UPDATE `groups` SET `group_name`=? WHERE `group_id`=? LIMIT 1")
        or die('MySQL Error. Error[' . $mysql->errno . ']: ' . $mysql->error);
    ...
    {
        $mysql = MySQL::GetInstance();
        $prep_query = $mysql->prepare("UPDATE `groups` SET `group_color`=? WHERE `group_id`=? LIMIT 1")
        or die('MySQL Error. Error[' . $mysql->errno . ']: ' . $mysql->error);
    ...
    {
        $mysql = MySQL::GetInstance();
        $prep_query = $mysql->prepare("UPDATE `groups` SET `group_package_price`=? WHERE `group_id`=? LIMIT 1")
        or die('MySQL Error. Error[' . $mysql->errno . ']: ' . $mysql->error);
    ...
    {
        $mysql = MySQL::GetInstance();
        $prep_query = $mysql->prepare("UPDATE `groups` SET `group_add_news`=?, `group_edit_news`=?, `group_delete_news`=?, `group_moderate_postages`=?, `group_delete_postages`=?, `group_refund_postages`=?, `group_edit_user`=?, `group_edit_money`=?, `group_edit_groups`=? WHERE `group_id`=? LIMIT 1")
        or die('MySQL Error. Error[' . $mysql->errno . ']: ' . $mysql->error);
    ...
    {
        $mysql = MySQL::GetInstance();
        $prep_query = $mysql->prepare("DELETE FROM `groups` WHERE `group_id`=?")
        or die('MySQL Error. Error[' . $mysql->errno . ']: ' . $mysql->error);
